 ...
 * a CSV
 * Information about the structure of the CSV file (also included in
...
public class PentominoDatabase {
    ...
    public static int[][][][] data = loadData();

    /**
     * Loads and decodes the CSV file
     *
     * @return array of pieces. Dimensions: 1-Piece ID; 2-Mutation; 3-X
    ...
    private static int[][][][] loadData() {
        ...
        // Open the CSV file
        File file = new File("src/Pentominoes/pentominos.csv");
        ...
        try {
            ...
            while (scanner.hasNextLine()) // For each line in the CSV file
            {
                ...
                if (Integer.parseInt(values[0]) > dynamicList.size() - 1) {
                    dynamicList.add(new ArrayList<>());
                }

                int xSize = Integer.parseInt(values[2]);
                int ySize = Integer.parseInt(values[3]);
                int[][] piece = new int[xSize][ySize];

                // Convert 1D list to 2D list
                for (int i = 0; i < xSize * ySize; i++) {
                    piece[i / ySize][i % ySize] = Integer.parseInt(values[4 + i]);
                }
